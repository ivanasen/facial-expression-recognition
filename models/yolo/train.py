...
anchors = utils.get_anchors("../data/wider_anchors.txt")
class_names = utils.get_classes("../data/wider_classes.txt")
classes_count = len(class_names)
input_shape = (608, 608)
ds_train = WiderDataset(
    "../data", input_shape[0], 1000, "train", classes_count, anchors)
...
def train_step(image_batch, target):
    with tf.GradientTape() as tape:
        pred_result = model(image_batch, training=True)
        ...
        if global_steps < warmup_steps:
            lr = global_steps / warmup_steps * config.TRAIN_LR_INIT
        else:
...
def train_yolo():
    for epoch in range(1):
        for image_data, target in ds_train:
